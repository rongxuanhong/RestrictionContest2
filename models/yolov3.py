...
class Upsample(nn.Module):
    ...
    def forward(self, input):
        return F.interpolate(input, scale_factor=self.scale_factor, mode=self.mode)
...
def create_yolov3_modules(config_model, ignore_thre):
    ...
    mlist.append(Upsample(scale_factor=2, mode='nearest'))
...
def test_yolo():
    ...
    with open('../config/yolov3_default.cfg', 'r') as f:
        cfg = yaml.load(f)
    yolo = YOLOv3(cfg['MODEL'])
    get_number_of_param(yolo)
